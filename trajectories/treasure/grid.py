...
if __name__ == '__main__' :
    ...
    5 actions 
    10 Skills
    Always has 6 segments
    Obs is state[0], 10x10x12
    ...
    # while tn < num_traces:
    #     env = ComPILEEnv(np.random.randint(1000))
    #     test = env.reset()
    #     acts = env.solve()

    #     #Only append the trajectory if it is of the correct length
    #     if len(acts) == trace_length:
    #         for i, a in enumerate(acts):
    #             state, reward, done, info = env.step(a)

    #         if done:
    #             tn += 1
    #             #Append the states and actions to the main array here
